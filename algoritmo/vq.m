...
% * nvector = n ́umero de code vectors requeridos %
%Parametros de salida:
% * code = code vectors resultantes
function code = vq(data,nvector) %distorsion epsilon->determina cu ́anto itero
deps = 1e-3;
[dim,n] = size(data);
%randomizo
...
%inicialmente separo el clusters al azar y %calculo los centroides correspondientes
m = floor(n/nvector);
for k = 1:nvector-1
    clus(k,n+1) = m;
...
%En cada paso reagrupo los clusters y calculo la nueva %distorsi ́on media. Si el cambio de distorsi ́on no es significativo %paro
olddist = 0;
newdist = +inf;
while abs(newdist-olddist) > deps
    ...
    for k = 1:nvector
        ...
        if(columns(mean(data(:,clus(k,1:m)).').')!= 0)
            code(:,k) = mean(data(:,clus(k,1:m)).').';
        endif;
    end 
end